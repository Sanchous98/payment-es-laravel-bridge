...
class SourceNormalizer implements NormalizerAwareInterface, NormalizerInterface, DenormalizerAwareInterface, DenormalizerInterface
{
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array
    {
        if (!$data instanceof SourceInterface) {
            throw new NotNormalizableValueException('Unsupported object type.');
        }

        return [
            'type' => $data::TYPE,
            $data::TYPE => $this->normalizer->normalize($data, $format, $context),
        ];
    }
    ...
    {
        return [SourceInterface::class => true, TokenizedSourceInterface::class => true];
    }
